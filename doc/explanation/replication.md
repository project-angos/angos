...
- **Coalescing** -- the queue deduplicates on a `lock_key` of `(operation, downstream, namespace, tag)`. A second push to the same tag while one is pending is a no-op, and a delete coalesces with other deletes for the same tag; but a push and a delete for the same tag carry different keys and do not collapse into each other. The handler re-resolves the **current** tag-to-digest mapping at execution time, so the latest content always wins regardless of how many enqueues coalesced.
