...
Running `--dry-run` reports the replication actions that would be taken without enqueuing anything: an `EnqueueReplicationPush` per diverging or downstream-missing tag, plus an `EnqueueReplicationDelete` per downstream-only tag on a `prune = true` downstream. Reconciliation is operator-driven (a systemd timer or Kubernetes CronJob); there is no in-server scheduler.
