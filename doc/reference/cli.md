...
- Reclaims the filesystem listings of layers no image an index policy applies to uses, the same pass as [`reconcile index`](#reconcile-index); images outside those repositories index again when opened.
...
Enqueue a scan job for every image manifest its repository's scan policy applies to, judged with the time of its newest report as `image.scanned_at`, so images pushed before scanning was enabled, whose scan failed past its retries, or whose report the rules find due get one; a scheduled run keeps reports current. The running server or a worker drains the jobs; the command returns once they are enqueued. See [Scan Images](../how-to/scan-images.md).

| Option      | Short | Description                                              |
|-------------|-------|----------------------------------------------------------|
| `--dry-run` | `-d`  | Preview what would be enqueued without changes           |
| `--force`   |       | Scan every image again, attaching a fresh report to each |
| `--concurrency <N>` | | Namespaces checked concurrently (default 25); each adds a small fixed tag-read fan-out when a policy has rules |

#### reconcile index

Enqueue a filesystem index job for every tar layer of the images an index policy applies to that has no listing yet, so the web UI opens them without an "Indexing" wait, and reclaim the listings of every other layer. A layer shared by several images is enqueued once, and kept while any image an index policy applies to uses it. Any image indexes itself the first time its filesystem is opened, so this is for having the listings ready ahead of that, or, with `--force`, for walking every layer again; the reclaim drops what those on-demand opens left behind in repositories without the flag, which index again when opened. The running server or a worker drains the jobs; the command returns once they are enqueued and the listings reclaimed. See [Explore Image Filesystems](../how-to/explore-image-filesystems.md).
...
# Give a scan to every image the scan policies apply to, new or due
