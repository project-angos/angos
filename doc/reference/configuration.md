...
| `url`                   | string   | required           | Downstream registry URL. A bare host (`http://host:8000`) mirrors the namespace verbatim; a path (`http://host:8000/team`) becomes the namespace prefix the content lands under, replacing the source repository prefix (`<repo>/x` → `team/x`). Angos serves the OCI API at the root, so the path is mapped into the namespace, not the HTTP path. |
