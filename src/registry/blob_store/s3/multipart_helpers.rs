...
use super::{Backend, UploadedPart};
use crate::{
    oci::Digest,
    registry::{
        blob_store::{Error, sha256_ext::Sha256Ext},
        path_builder,
    },
};
...
impl Backend {
    ...
    pub async fn ensure_multipart_upload(
        ...
        let upload_id = self.store.create_multipart_upload(upload_path).await?;
    ...
    pub async fn copy_staged_object_to_blob_and_cleanup(
        ...
        let blob_path = path_builder::blob_path(digest);
        self.store.copy_object(source_path, &blob_path).await?;
    ...
    pub async fn handle_upload_task_failure(
        ...
        if let Err(abort_err) = self
            .store
            .abort_multipart_upload(upload_path, upload_id)
            .await
        {
...
pub fn uploaded_size(parts: &[UploadedPart]) -> u64 {
